...
trait XIForm{
		...
		foreach ($data as $key => $value) {
			...
			switch (@$value['type']) {
				case 'delete':
					...
					$buttons .= 
			                    Form::open(['method' => 'DELETE', 'url' => $value['action'], 'class'=>'validate-form', 'data-confirm'=>'Anda yakin menghapus data ini?']).
			                    Form::submit($title, ['class' => 'btn btn-danger btn-block', 'style' => 'text-align:left']).
			                    Form::close();
					break;
	...
	protected static function icon_button($data, $isGroup=false, $btn_label='Action')
	{
		...
		foreach ($data as $key => $value) {
			...
			$form .= (!empty($value['validation'])) ? JsValidator::formRequest($value['validation'], '.swal2-container.swal2-shown .form-container') : '';
			switch (@$value['type']) {
				case 'reject':
				case 'delete':
